...
if not WX_AVAILABLE:
    ...
    class GUIController:
        ...
        def __init__(self, main_window: MainWindow, services: Services, event_bus: EventBus):
            """
        ...
        def _bind_events(self):
            ...
            if self.main_window.menu_initialize:
                self.main_window.bind_menu_event(
                    self.main_window.menu_initialize.GetId(), self._on_initialize_system
                )
            if self.main_window.menu_refresh:
                self.main_window.bind_menu_event(
                    self.main_window.menu_refresh.GetId(), self._on_refresh
                )
            if self.main_window.menu_network:
                self.main_window.bind_menu_event(
                    self.main_window.menu_network.GetId(), self._on_network
                )
            if self.main_window.menu_generate:
                self.main_window.bind_menu_event(
                    self.main_window.menu_generate.GetId(), self._on_generate_coins
                )

            # Toolbar events
            if hasattr(self.main_window, "send_tool_id"):
                self.main_window.bind_tool_event(self.main_window.send_tool_id, self._on_send)
            if hasattr(self.main_window, "addr_tool_id"):
                self.main_window.bind_tool_event(
                    self.main_window.addr_tool_id, self._on_address_book
                )
        ...
        def _on_initialize_system(self, event):
            ...
            def init_thread():
                try:
                    ...
                    if not result:
                        wx.CallAfter(
                            self.main_window.set_status_text, f"Wallet load error: {result.error}"
                        )
                    ...
                    wx.CallAfter(self._refresh_transactions)
                    wx.CallAfter(self._update_status)
                    wx.CallAfter(self.main_window.set_status_text, "System initialized")
                    self.main_window.system_initialized = True
                except Exception as e:
                    wx.CallAfter(
                        self.main_window.set_status_text, f"Initialization error: {str(e)}"
                    )
                ...
                try:
                    if is_checked:
                        ...
                        if hasattr(self.services.network_service, "start"):
                            ...
                            if result:
                                wx.CallAfter(self.main_window.set_status_text, "Network started")
                            else:
                                wx.CallAfter(
                                    self.main_window.set_status_text,
                                    f"Network start error: {result.error}",
                                )
                                wx.CallAfter(self.main_window.menu_network.Check, False)
                        else:
                            wx.CallAfter(
                                self.main_window.set_status_text,
                        ...
                        if hasattr(self.services.network_service, "stop"):
                            ...
                            wx.CallAfter(self.main_window.set_status_text, "Network stopped")
                        else:
                            wx.CallAfter(
                                self.main_window.set_status_text,
                                "Network service not yet implemented",
                    ...
                    wx.CallAfter(self.main_window.set_status_text, f"Network error: {str(e)}")
            ...
            def mining_thread():
                try:
                    if is_checked:
                        ...
                        if result:
                            wx.CallAfter(self.main_window.set_status_text, "Mining started")
                        else:
                            wx.CallAfter(
                                self.main_window.set_status_text,
                                f"Mining start error: {result.error}",
                            )
                            wx.CallAfter(self.main_window.menu_generate.Check, False)
                    else:
                        # Stop mining
                        self.services.mining_service.stop_mining()
                        wx.CallAfter(self.main_window.set_status_text, "Mining stopped")
                except Exception as e:
                    wx.CallAfter(self.main_window.set_status_text, f"Mining error: {str(e)}")
        ...
        def _refresh_transactions(self):
            ...
            try:
                ...
                sorted_txs = sorted(
                    transactions, key=lambda wtx: getattr(wtx, "n_time_received", 0), reverse=True
                )
        ...
        def _get_peer_count(self) -> int:
            ...
            try:
                if (
                    hasattr(self.services.network_service, "network_manager")
                    and self.services.network_service.network_manager
                ):
                    return (
                        self.services.network_service.network_manager.network_state.get_peer_count()
                    )
                return 0
            except Exception:
                return 0
