...
# Genesis block hash - mainnet default
    ...
    if mode == "private":
        try:
            ...
            if HASH_GENESIS_BLOCK_PRIVATE:
                ...
                print("Warning: Private genesis block not yet created, using mainnet")
        except ImportError:
...
def create_private_genesis_block():
    """
    Create a custom genesis block for the private network.

    Returns:
        Block: The private network genesis block
    """
    try:
    ...
    # Create output with custom pubkey for private network
    tx_new.vout = [TxOut()]
    ...
    block.version = 1
    block.time = GENESIS_BLOCK_TIME_PRIVATE
    block.bits = PRIVATE_NETWORK_BITS
    block.nonce = 0  # Will be set by mining

    # Mine the genesis block (find nonce)
    print("Mining private network genesis block...")
    target = block.get_target()

    while True:
        block_hash = block.get_hash()
        if block_hash <= target:
            print(f"Genesis block mined! Nonce: {block.nonce}")
            print(f"Genesis block hash: {block_hash.get_hex()}")
            break
        block.nonce += 1
        if block.nonce % 100000 == 0:
            print(f"Tried nonce {block.nonce}...")
...
def initialize_private_network(blockchain_service=None):
    ...
    This should be called once to set up the private network.

    Args:
        blockchain_service: Optional BlockchainService to add genesis block to state

    Returns:
        Block: The private network genesis block
    """
    try:
        genesis_block = create_private_genesis_block()
        ...
        if blockchain_service:
            ...
            if chain.accept_block(genesis_block):
                print(f"Genesis block added to chain")
            else:
                print(f"Warning: Failed to add genesis block to chain")

            # Then add to new blockchain_state
            # Get the height from the chain
            block_index = chain.get_block_index(genesis_block.get_hash())
            height = block_index.height if block_index else 0

            if blockchain_service.blockchain_state.add_block(genesis_block, height=height):
                print(f"Genesis block added to blockchain state")
            else:
                print(f"Warning: Failed to add genesis block to state")

        print(f"Private network initialized with genesis block: {genesis_hash.get_hex()}")
